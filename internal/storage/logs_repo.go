...
func (r *LogsRepo) Insert(entry log.Entry) (int64, error) {
	...
	res, err := r.db.Exec(
		`INSERT INTO logs (route_name, method, path, request_url, target_url, protocol, status_code, latency_ms,
		 req_headers, req_body, resp_headers, resp_body, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		entry.RouteName, entry.Method, entry.Path, entry.RequestURL, entry.TargetURL, entry.Protocol,
...
func (r *LogsRepo) Query(filter log.Filter) (log.QueryResult, error) {
	...
	dataQuery := fmt.Sprintf("SELECT id, route_name, method, path, request_url, target_url, protocol, status_code, latency_ms, req_headers, req_body, resp_headers, resp_body, created_at FROM logs%s ORDER BY id DESC LIMIT ? OFFSET ?", clause)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.RouteName, &e.Method, &e.Path, &e.RequestURL, &e.TargetURL, &e.Protocol,
...
func (r *LogsRepo) Get(id int64) (*log.Entry, error) {
	...
	var createdAt time.Time
	err := r.db.QueryRow(
		"SELECT id, route_name, method, path, request_url, target_url, protocol, status_code, latency_ms, req_headers, req_body, resp_headers, resp_body, created_at FROM logs WHERE id=?", id,
	).Scan(&e.ID, &e.RouteName, &e.Method, &e.Path, &e.RequestURL, &e.TargetURL, &e.Protocol,
		&e.StatusCode, &e.LatencyMs, &e.ReqHeaders, &e.ReqBody,
